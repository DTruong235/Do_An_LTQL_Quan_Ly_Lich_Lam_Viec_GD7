                    ...
                    try
                    {
                        ...
                        if (workbook.Worksheets.Count > 2)
                        {
                            ...
                            foreach (var row in rowsCT)
                            {
                                try
                                {
                                    int idPC_Excel = row.Cell(1).GetValue<int>();
                                    string tenViec = row.Cell(2).GetValue<string>();
                                    ...
                                    if (mapID.ContainsKey(idPC_Excel))
                                    {
                                        var ct = new Chi_Tiet_Cong_Viec
                                        {
                                            MaPhanCong = mapID[idPC_Excel], // Lấy ID mới được sinh ra trong DB
                                            TenDauViec = tenViec.Trim(),
                                            TrangThai = row.Cell(3).GetValue<string>() == "Xong" // Chuyển chữ thành bool
                                        };
        ...
        public void ExportLichLamViec(string filePath, ListBox lstLog)
        {
            try
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    string[] header3 = { "Mã Phân Công", "Mã Chi Tiết", "Mã Phân Công", "Tên Đầu Việc", "Trạng Thái" };
                    ...
                    for (int i = 0; i < listCT.Count; i++)
                    {
                        ...
                        wsChiTiet.Cell(r, 2).Value = listCT[i].MaPhanCong;
                        wsChiTiet.Cell(r, 3).Value = listCT[i].TenDauViec;
                        wsChiTiet.Cell(r, 4).Value = listCT[i].TrangThai ? "Hoàn thành" : "Chưa hoàn thành";
