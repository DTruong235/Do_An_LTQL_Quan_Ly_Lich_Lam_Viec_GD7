...
{
    public partial class frmTienDoCongViec : Form
    {
        ...
        private void ThietLapBinding()
        {
            ...
            cboTenLich.DataBindings.Clear();
            cboTenLich.DataBindings.Add("SelectedValue", tienDoBindingSource, "MaLich", true, DataSourceUpdateMode.OnPropertyChanged);
        }
        ...
        private void LoadComboBoxLich()
        {
            ...
            {
                // Nếu là Nhân viên (Quyền 0): Chỉ thấy lịch mình được phân công
                if (Program.CurrentUser.MaQuyen == 0)
                {
                    query = query.Where(l => l.Chi_Tiet_Phan_Cong
                                 .Any(p => p.MaNhanVien == Program.CurrentUser.MaNhanVien));
                }
            }
            else
            {
        ...
        private void CapNhatTienDo()
        {
            ...
            lblTrangThai.ForeColor = (phanTram == 100) ? Color.Green : Color.Black;
        }
